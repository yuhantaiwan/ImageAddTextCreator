	...
	var MemeGenerator = function(element, options){
		...
		this.ui = {
			createControls: function(){
				...
				controls.append(MG.ui.createTextBox(i18n.middleTextPlaceholder, "center center"));
				...
				$(addTextboxButton).click(function(e){
					...
					var newTextbox = MG.ui.createTextBox("請輸入文字", "top center").insertAfter(controls.find(".mg-textbox").last());
					...
					newTextbox.find("input[type]").first().focus();
			...
			createTextBox: function(placeholder, position, params){
				...
				// box.append($('<textarea class="mg-textbox-text" rows="" cols="" placeholder="' + placeholder + '" data-text="' + params.text + '" autofocus maxlength="20" spellcheck="false"></textarea>'));
				box.append($('<input type="text" class="mg-textbox-text" placeholder="' + placeholder + '" data-text="' + params.text + '" value="' + params.text + '">'));
				box.append($('<input type="range" class="mg-textbox-size" value="' + params.fontSize + '" step="' + MG.settings.fontSizeStep + '" min="' + MG.settings.minFontSize + '" max="' + MG.settings.maxFontSize + '">'));
				...
				{
					box.append('<input type="color" class="mg-textbox-text-color colorpicker" value="' + params.color + '">');
					box.append('<input type="color" class="mg-textbox-border-color colorpicker" value="' + params.borderColor + '">');
					box.find(".colorpicker").wrap('<div class="colorpickerContainer"></div>');
					
					MG.ui._bindColorpicker(box.find(".colorpicker"));
				}
				
				box.append($('<input type="range" class="mg-textbox-border-width" value="' + params.borderWidth + '" step="' + MG.settings.borderWidthStep + '" min="' + MG.settings.minBorderWidth + '" max="' + MG.settings.maxBorderWidth + '">'));
				
				// box.append($('<button class="btn deleteBtn" onclick="deleteTxt()">刪除</button>'));
				...
				switch(type)
				{
					case 'text':
						layers.push({
							...
							// text: layer.find(".mg-textbox-text").attr("data-text").replace(/<br>/gi,"\n"),
							x: layer.attr("data-x"),
							y: layer.attr("data-y"),
